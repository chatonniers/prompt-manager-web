...
export default function AdminMcpCard() {
  ...
  const [editVal, setEditVal] = useState({ label: '', clientId: '', clientSecret: '' });
  const [showSecret, setShowSecret] = useState({});
  const [newCred, setNewCred] = useState({ label: '', clientId: '', clientSecret: '' });
  ...
  async function handleAdd() {
    ...
    setNewCred({ label: '', clientId: '', clientSecret: '' });
  ...
  return (
    <div className="view-card admin-card">
        ...
        {creds.map((cred, idx) => (
          <div key={cred.id} className="admin-row">
            ...
            {editingIdx === idx ? (
              ...
              <>
                <div className="admin-mcp-display" onClick={() => { setEditingIdx(idx); setEditVal({ label: cred.label || '', clientId: cred.clientId || '', clientSecret: cred.clientSecret || '' }); }}>
                  <span className="admin-mcp-label">🔑 {cred.label || cred.clientId || '(unnamed)'}</span>
                  <span className="admin-mcp-id">{cred.clientId}</span>
                </div>
      ...
      <div className="admin-mcp-add">
        <input className="admin-item-input" style={{ flex: 1, border: '1.5px dashed var(--pm-accent)', borderRadius: 7, padding: '6px 10px', background: 'var(--pm-bg)' }} type="text" value={newCred.label} onChange={e => setNewCred(p => ({ ...p, label: e.target.value }))} placeholder="Label (e.g. DEV, PROD)…" />
        <input className="admin-item-input" style={{ flex: 2, border: '1.5px dashed var(--pm-accent)', borderRadius: 7, padding: '6px 10px', background: 'var(--pm-bg)' }} type="text" value={newCred.clientId} onChange={e => setNewCred(p => ({ ...p, clientId: e.target.value }))} placeholder={t('mcpClientId', lang) + '…'} />
        <div style={{ flex: 2, display: 'flex', gap: 4 }}>
          <input className="admin-item-input" style={{ flex: 1, border: '1.5px dashed var(--pm-accent)', borderRadius: 7, padding: '6px 10px', background: 'var(--pm-bg)' }} type={showNewSecret ? 'text' : 'password'} value={newCred.clientSecret} onChange={e => setNewCred(p => ({ ...p, clientSecret: e.target.value }))} placeholder={t('mcpClientSecret', lang) + '…'} onKeyDown={e => e.key === 'Enter' && handleAdd()} />
